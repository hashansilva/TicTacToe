...
public class Game {
    ...
    public String checkGameWinner(char [][]grid){
        ...
        for(int i=0; i<3;i++){
            String value = String.valueOf(grid[i][0]);
            if (!empty.equals(value) && grid[i][0] == grid[i][1] && grid[i][1] == grid[i][2]) {
                return value+ " wins";
            }
        }
        for(int i=0; i<3;i++){
            String value = String.valueOf(grid[0][i]);
            if (!empty.equals(value)&& grid[0][i] == grid[1][i] && grid[1][i] == grid[2][i]) {
                return value+ " wins";
            }
        }
        String value = String.valueOf(grid[1][1]);
        if (!empty.equals(value) && grid[0][2] == grid[1][1] && grid[1][1] == grid[2][0]) {
            return value+ " wins";
        }
        if (!empty.equals(value) && grid[0][0] == grid[1][1] && grid[1][1] == grid[2][2]) {
            return value+ " wins";
        }
        if (freeSpots == 0) {
            return "Tie";
